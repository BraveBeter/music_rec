...
from app.schemas.recommendation import RecommendationResponse, SimilarRecommendationResponse
...
@router.get("/similar", response_model=SimilarRecommendationResponse)
async def get_similar(
    ...
    Returns empty source_tracks and items if user has no play history.
    """
    result = await get_similar_recommendations(db=db, user_id=current_user.user_id)
    return SimilarRecommendationResponse(**result)
