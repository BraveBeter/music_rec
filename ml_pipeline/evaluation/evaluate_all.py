...
def _build_play_sequences(df):
    plays = df[df["interaction_type"].isin([1, 2])].sort_values("created_at")
    return {
        int(user_id): group["track_id"].tolist()
        for user_id, group in plays.groupby("user_id")
        if len(group) >= 3
    }
...
def _build_sasrec_fn(sasrec_model, train_df, val_df=None, top_k=20):
...
def _build_deepfm_fn(deepfm_model, feature_meta, user_features, item_features,
    ...
    def recommend(user_id):
        ...
        scores = deepfm_model.predict(sparse_array, dense_array) + np.array(prior_bonus, dtype=np.float32)
...
def main():
    ...
    sasrec = _train_sasrec(_build_play_sequences(train), track2idx)
    ...
    sasrec_fn = _build_sasrec_fn(sasrec, train, data["val"], top_k=20)
